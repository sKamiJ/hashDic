...
/**
 * 原文生成器T3型，生成7-10位数字组合的字符串
 * 
 * @author KamiJ
 *
 */
public class TextCreatorT3 extends TextCreator {
	/**
	 * 原文生成器类型
	 */
	 ...
	 * 字符库
	 */
	public static final char[] CHAR_LIB = "0123456789".toCharArray();
	 ...
	 * 该原文生成器起始原文
	 */
	public static final String TEXT_FI = "0000000";

	/**
	 * 该原文生成器终原文
	 */
	public static final String TEXT_LA = "00000000000";
	 ...
	 * 新建原文生成器，从指定的原文开始
	 */
	public TextCreatorT3(String text) {
		int len = text.length();
		// 检查原文长度是否合法
		if ((len <= 6 || len >= 11) && !text.equals(TEXT_LA))
	...
	/**
	 * 原文到达n+7位时，所包含的n+6位原文总数，n为数组位置
	 */
	private static final long[] TEXT_NUM = new long[5];
	static {
		TEXT_NUM[0] = 0;
		for (int i = 0; i < 4; i++)
			TEXT_NUM[i + 1] = (long) (TEXT_NUM[i] + Math.pow(CHAR_LIB.length, i + 7));
	}
	 ...
	 * 原文总数
	 */
	public static final long TEXT_SUM = TEXT_NUM[4];

	@Override
	public TextData getData() {
		lockRead();
		long completeTextNum = TEXT_NUM[text.length() - 7] + getTextIndex(text);
